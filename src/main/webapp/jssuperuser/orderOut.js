Ext.onReady(function() {
   ...
   var orderOutNo2 = null;

    var sm = new Ext.grid.CheckboxSelectionModel({handleMouseDown:Ext.emptyFn()}); // CheckBox
    var cm = new Ext.grid.ColumnModel([
        new Ext.grid.RowNumberer(), //
        sm,
        {header: "出库单编号", dataIndex: 'oderoutno',width:100,sortable:true,renderer:renderusername},
        ...
        {header: "出库日期", dataIndex: 'tradeoutdate',width:100,sortable:true,renderer:Ext.util.Format.dateRenderer('Y-m-d')}
    ]);
    //var ds = new Ext.data.Store({
    var ds = new Ext.data.GroupingStore({
        proxy: new Ext.data.HttpProxy({url: 'orderOut/listOrderOut'}),
        reader: new Ext.data.JsonReader({
            totalProperty: 'total',
            root: 'orderOutList',
            successProperty: 'success'
      }, [
            {name: 'oderoutno', mapping: 'oderoutno', type: 'string'},
            ...
            {name: 'tradeoutdate', mapping: 'tradeOut.tradeoutdate', type: 'date',dateFormat:'time'}
        ]) ,
...
//        autoExpandColumn:'regionname',
    ...
    ds.load({params: {start: 0, limit:27,name:orderOutNo2}});

    ds.on('beforeload', function() {
        var orderOutNo2 = Ext.getCmp('comboid').getRawValue();
         this.baseParams = {start:0,limit:27,name:orderOutNo2};
    });
...
function renderusername(value){
    ...
    return "<span style='color:#FF3333;font-weight:bold;'>"+catgoryno+"</span>";
      ...
      function searchadmin(){
         ...
         if(name2.length<=0 || name2.length == ''){
             Ext.Msg.alert('提示','请选择一个你要查询的类目名');
                 ...
                 Ext.getCmp('orderOutGrid').getStore().load();
         }
     }
     ...
     var batchAddOderInWin = new Ext.Window({
         ...
         html: '<iframe style="width:100%; height:450px;" src="common/addOrderOut.jsp" frameborder="0" scrolling="false"></iframe>',
     });
