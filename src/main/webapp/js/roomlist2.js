Ext.onReady(function() { 
   ...
   var cookieArr = window.document.cookie.split(";");   
    ...
    Ext.QuickTips.init();  
    var regionid1=Ext.get("regionid").dom.value;//获取flatid值
    ...
    //栋数据源
     var flatStore = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
           url: 'flatlist2.action'
        }),
        reader: readerflat,
        remoteSort: true,
        baseParams: { start:0,limit: 20, regionid: regionid1 } //
    });

    flatStore.on("beforeload", function() {
        flatStore.baseParams = { regionid: regionid1 };
    });
    ...
    var cm = new Ext.grid.ColumnModel([   
        ...
        sm,   
        {header: "栋名", dataIndex: 'flatname',width:100,sortable:true},
        {header: "房号", dataIndex: 'roomid',width:80,sortable:true,renderer:renderroomid},
        {header: "系别", dataIndex: 'sdeptname',width:120,sortable:true},
    ...
    var ds = new Ext.data.GroupingStore({
        proxy: new Ext.data.HttpProxy({url: 'roomlist2.action'}),   
    ...
    var grid = new Ext.grid.EditorGridPanel({
       region:'center', 
        ...
        bbar: new Ext.PagingToolbar({   
           ...
           emptyMsg: '没有记录' ,
           items:['-',{text:'请输入你要查找的房号:'},flatcombo,numbercombo1,numbercombo2,numbercombo3,{text:'至'},numbercombo4,numbercombo5,numbercombo6,search,'-'] 
        })   
    });   
    
    ds.load({params: {start: 0, limit:21,regionid:regionid1,flatname:flatname1,roomid1:roomid11,roomid2:roomid12}}); 
    ...
    ds.on('beforeload', function() {
    	...
    	var flatname1 = Ext.getCmp('flatcomboid').getRawValue();
    	
    	 this.baseParams = {start: 0, limit:21,regionid:regionid1,flatname:flatname1,roomid1:roomid11,roomid2:roomid12};
    });
    
    //ds.load(/*{params: {start: 0, limit:10,flatid:flatid}}*/); 
    
    var roomlist1panel = new Ext.Panel({
	      ...
	      var numbercombo4 = new Ext.form.ComboBox({
		    ...
		    //查询组件
		      ...
		      function searchroom(){
		    	...
		    	var flatname = Ext.getCmp('flatcomboid').getRawValue().trim();
		    	
		    	if( (roomid1.length<=0 || roomid1 == '') && (roomid2.lenght<=0 || roomid2 == '') && (flatname.length<=0 || flatname == '')){
		    		Ext.Msg.alert('提示','请选择一个你要查找的关键词');
		    		
		    	}else{
		    		if( (roomid1.length>=0 || roomid1 != '') && (roomid2.lenght>=0 || roomid2 != '') && (flatname.length<=0 || flatname == '')){
		    			Ext.Msg.alert('提示','请选择你要查找的栋名');
		    		}else{
